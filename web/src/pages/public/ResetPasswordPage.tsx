...
export function ResetPasswordPage() {
  ...
  const resetMutation = useMutation({ mutationFn: () => api.post('/auth/reset-password', { token, password }), onSuccess: () => setSuccess(true), onError: (err: unknown) => setFormError(apiErrorMessage(err, t)) })
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 px-6 auth-grid-bg">
      <div className="w-full max-w-sm">
        <div className="mb-8"><span className="text-sm font-semibold text-zinc-50 tracking-tight">AgentOrbit</span></div>
        {success ? (
