  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 px-6 auth-grid-bg">
      <div className="w-full max-w-sm text-center">
        <div className="mb-6"><span className="text-sm font-semibold text-zinc-50 tracking-tight">AgentOrbit</span></div>
        ...
        {verifyMutation.isError && <div className="space-y-4"><p className="text-sm text-zinc-400">{apiErrorMessage(verifyMutation.error, t)}</p><Link to="/login" className="text-sm text-zinc-500 hover:text-zinc-300 block">{t.auth_verify_back}</Link></div>}
      </div>
    </div>
  )
}
