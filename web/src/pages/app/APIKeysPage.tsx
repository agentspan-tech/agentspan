...
export function APIKeysPage() {
  ...
  const dismissRawKey = useCallback(() => { setRawKeyResult(null); setCopied(false); setKeyCountdown(30); setRawKeyTest({ loading: false }) }, [])

  async function handleRawKeyTest() {
    if (!rawKeyResult) return
    setRawKeyTest({ loading: true })
    try {
      const result = await testKey.mutateAsync({ keyID: rawKeyResult.id })
        ...
        <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-50">
          ...
          <DialogFooter>
            <button
              ...
              disabled={rawKeyTest.loading || !!rawKeyTest.result}
