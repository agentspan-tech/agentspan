...
const ANOMALY_STYLES: Record<string, string> = {
  hallucination: 'text-red-400 bg-red-500/10',
  context_loss: 'text-orange-400 bg-orange-500/10',
  echo: 'text-amber-400 bg-amber-500/10',
  off_topic: 'text-purple-400 bg-purple-500/10',
  empty_output: 'text-zinc-400 bg-zinc-500/10',
  malformed: 'text-yellow-400 bg-yellow-500/10',
}

function anomalyCategoryBadge(category?: string) {
  if (!category) return null
  const style = ANOMALY_STYLES[category] ?? 'text-yellow-400 bg-yellow-500/10'
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded ${style}`}>
...
function spanColor(span: SpanItem): string {
  ...
  if (span.anomaly_reason) return 'bg-yellow-500/70'
...
export function SessionDetailPage() {
  ...
  const timelineData = session?.spans && session.spans.length >= 1 ? (() => {
    // Compute true start (min started_at) and true end (max started_at + duration_ms)
    // across all spans — don't assume spans[0] is the earliest.
    let start = Infinity
    let end = 0
    for (const sp of session.spans) {
      const spStart = new Date(sp.started_at).getTime()
      if (spStart < start) start = spStart
      const spEnd = spStart + Math.max(sp.duration_ms, 1)
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
                ...
                >
                  ...
                  {session.spans.map((span, i) => {
                    ...
                    const spanStart = new Date(span.started_at).getTime()
                    const rawLeft = ((spanStart - timelineData.start) / timelineData.totalMs) * 100
                ...
                <div className="flex flex-wrap gap-3 mt-3 text-[10px] text-zinc-600">
                  ...
                  <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-yellow-500/70" /> {t.detail_legend_anomaly}</span>
                </div>
              </div>
            )
          })()}
...
const SYSTEM_PROMPT_RE = /\[System Prompt #(SP-\d+)\]/

function SpanInputText({ input, systemPromptId }: { input: string; systemPromptId?: string }) {
  const match = SYSTEM_PROMPT_RE.exec(input)
  if (!match || !systemPromptId) return <>{input}</>

  const before = input.slice(0, match.index)
  const tag = match[0]
  const after = input.slice(match.index + tag.length)

  return (
    <>
      {before}
      <Link
        to={`/system-prompts/${systemPromptId}`}
...
function SpanRow({ span, index, gapMs }: { span: SpanItem; index: number; gapMs: number }) {
  ...
  return (
    <div className="relative pl-6 pb-5 -ml-px animate-fade-in-up" style={{ animationDelay: `${Math.min(index * 40, 400)}ms` }}>
      ...
      <div className={`absolute -left-[5px] ${gapMs > 0 ? 'top-5' : 'top-3'} w-2 h-2 rounded-full ${span.anomaly_reason ? 'bg-yellow-400' : httpStatusDot(span.http_status)} ring-2 ring-zinc-950`} />
      <button
        type="button"
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {anomalyCategoryBadge(span.anomaly_category)}
                </div>
        ...
        <div className="mt-2 space-y-2 ml-6 animate-fade-in">
          {span.input && (
            <div>
              ...
              <pre className="bg-zinc-950 border border-zinc-800 rounded-md p-3 text-xs text-zinc-400 font-mono overflow-x-auto max-h-80 overflow-y-auto whitespace-pre-wrap break-words leading-relaxed"><SpanInputText input={span.input} systemPromptId={span.system_prompt_id} /></pre>
            </div>
          )}
          {span.output && (
            <div>
          ...
          {span.anomaly_reason && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-md bg-yellow-500/5 border border-yellow-500/20">
              ...
              <p className="text-xs text-yellow-400/90 leading-relaxed">{span.anomaly_reason}</p>
            </div>
          )}
        </div>
      )}
