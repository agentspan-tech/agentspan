...
import { ExportSplitButton } from '@/components/app/ExportSplitButton'

function SortHeader({ column, label, current, order, onToggle, className = '' }: {
  column: string
  label: string
  current: string
  order: string
  ...
  return (
    <th
      className={`px-5 py-3 text-xs font-medium uppercase tracking-wider select-none cursor-pointer group transition-colors ${isActive ? 'text-zinc-300' : 'text-zinc-600 hover:text-zinc-400'} ${className}`}
...
export function SessionsPage() {
  ...
  const hasFilters = !!(status || agentName || apiKeyID || providerType || sortBy)
    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-1">
          ...
          <ExportSplitButton
            orgID={activeOrgID}
            filters={{ status, agent_name: agentName, api_key_id: apiKeyID, provider_type: providerType }}
          />
        </div>
      </div>
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-zinc-800/60">
                  ...
                  <SortHeader column="span_count" label={t.sessions_col_spans} current={sortBy} order={sortOrder} onToggle={toggleSort} />
                  <SortHeader column="total_cost_usd" label={t.sessions_col_cost} current={sortBy} order={sortOrder} onToggle={toggleSort} />
                  <SortHeader column="started_at" label={t.sessions_col_started} current={sortBy} order={sortOrder} onToggle={toggleSort} />
                  <SortHeader column="last_span_at" label={t.sessions_col_activity} current={sortBy} order={sortOrder} onToggle={toggleSort} className="hidden lg:table-cell" />
                </tr>
              </thead>
