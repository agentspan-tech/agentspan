...
client = OpenAI(
  base_url="https://api.agentspan.tech/v1",
  api_key="al-your-key",
)`,
  },
  anthropic: {
...
client = anthropic.Anthropic(
  ...
  -d '{"model":"gpt-4o","messages":[...]}'`,
    after: `curl https://api.agentspan.tech/v1/chat/completions \\
...
function highlight(code: string, changed: 'before' | 'after') {
  ...
  return lines.map((line, i) => {
    const isChanged =
      ...
      (changed === 'after'  && (line.includes('api.agentspan.tech') || line.includes('api_key="al-') || line.includes('Authorization: Bearer al-')))
      ...
      <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-zinc-800">
        ...
        <div>
          <div className="px-5 py-2 border-b border-zinc-800 flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500 font-mono">after · AgentSpan</span>
          </div>
          <pre className="py-4 text-xs sm:text-sm font-mono leading-5 sm:leading-6 overflow-x-auto">
