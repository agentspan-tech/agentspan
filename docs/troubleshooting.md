...
- To check migration state: `docker compose exec postgres psql -U agentspan -c "SELECT * FROM schema_migrations"`
...
- Check the `Authorization: Bearer as-<key>` header is set correctly
