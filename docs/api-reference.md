...
AgentOrbit exposes three API surfaces: the public Dashboard API, the Proxy endpoints, and the Internal API (proxy-to-processing).
...
- **API key** (agents): `Authorization: Bearer ao-<key>` header. Created via the dashboard.
...
**Required header:** `Authorization: Bearer ao-<your-api-key>`

**Optional headers:**
- `X-AgentOrbit-Session: <id>` -- explicit session grouping
- `X-AgentOrbit-Agent: <name>` -- override agent name for this request
