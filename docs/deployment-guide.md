...
| `DATABASE_URL` | PostgreSQL connection string | `postgres://user:pass@postgres:5432/agentspan?sslmode=disable` |
...
```bash
# Single origin
ALLOWED_ORIGINS=https://agentspan.example.com

# Multiple origins
ALLOWED_ORIGINS=https://agentspan.example.com,https://app.example.com
```
...
docker compose exec postgres pg_dump \
  -U ${POSTGRES_USER:-agentspan} \
  -d ${POSTGRES_DB:-agentspan} \
  --format=custom \
  --compress=zstd \
  > agentspan_$(date +%Y%m%d_%H%M%S).dump
```
...
```cron
0 3 * * * docker compose -f /path/to/docker-compose.yml exec -T postgres pg_dump -U agentspan -d agentspan --format=custom --compress=zstd > /backups/agentspan_$(date +\%Y\%m\%d).dump 2>> /var/log/agentspan-backup.log
```
...
docker compose exec postgres pg_isready -U agentspan
...
```bash
docker compose exec postgres psql -U agentspan -d agentspan -c "
...
When restoring a backup from an older version onto a newer AgentSpan release:
...
server {
    listen 443 ssl;
    server_name agentspan.example.com;
...
server {
    listen 443 ssl;
    server_name proxy.agentspan.example.com;
