...
const testPhone = "+7 938 329-38-38"

// maskedPhone is the expected masked form of testPhone: first digit '7', middle '1's, last digit '1' (counter=1).
// Format is preserved: "+7 111 111-11-11"
const maskedPhone = "+7 111 111-11-11"
const testInputWithPhone = `{"model":"gpt-4","messages":[{"role":"user","content":"Call me at +7 938 329-38-38 please"}]}`
...
func TestProxyMasking_OffMode(t *testing.T) {
	...
	provider := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(200)
		fmt.Fprint(w, `{"id":"1","choices":[{"message":{"content":"Sure, calling +7 938 329-38-38"}}]}`)
	})
...
func TestProxyMasking_LLMOnly(t *testing.T) {
	...
	provider := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"id":"1","choices":[{"message":{"content":"Sure, calling +7 111 111-11-11"}}]}`)
	})
...
func TestProxyMasking_LLMStorage_OutputRemasking(t *testing.T) {
	...
	provider := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"id":"1","choices":[{"message":{"content":"I will call +7 111 111-11-11 now"}}]}`)
	})
...
func TestProxyMasking_FailOpen(t *testing.T) {
	...
	result, ok := safeMask(nil, masking.MaskingConfig{Phone: masking.MaskModeLLMOnly}, defaultMaskers)
	if !ok {
