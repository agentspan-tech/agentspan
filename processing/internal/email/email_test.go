...
func TestNewMailer_LogMailer(t *testing.T) {
	m := NewMailer(MailConfig{AppBaseURL: "http://localhost:3000"})
	if m.IsSMTP() {
...
func TestLogMailer_SendVerification(t *testing.T) {
	...
	if link != "http://app.test/auth/verify?token=tok123" {
...
func TestLogMailer_SendPasswordReset(t *testing.T) {
	...
	if link != "http://app.test/auth/reset-password?token=reset123" {
		t.Errorf("link = %q", link)
	}
}
