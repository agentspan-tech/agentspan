...
// GenerateAPIKey generates a new AgentOrbit API key.
// Returns the raw key (34 chars, "ao-" + 32 hex), its HMAC-SHA256 digest, and a display string.
func GenerateAPIKey(hmacSecret string) (raw string, digest string, display string, err error) {
	b := make([]byte, 16)
	if _, err = io.ReadFull(rand.Reader, b); err != nil {
		return "", "", "", fmt.Errorf("generate api key: %w", err)
	}
	raw = "ao-" + hex.EncodeToString(b)
