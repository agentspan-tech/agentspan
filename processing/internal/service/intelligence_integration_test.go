...
func TestIntelligenceService_ExtractSystemPrompts_ShortPrefix(t *testing.T) {
	...
	sessions, err := service.NewDashboardService(queries).ListSessions(ctx, org.ID, service.ListSessionsParams{})
	if err != nil {
		t.Fatalf("list sessions: %v", err)
	}
	sessionID := sessions.Sessions[0].ID
...
func TestIntelligenceService_ExtractSystemPrompts_SingleSpan(t *testing.T) {
	...
	sessions, _ := service.NewDashboardService(queries).ListSessions(ctx, org.ID, service.ListSessionsParams{})
	sessionID := sessions.Sessions[0].ID
