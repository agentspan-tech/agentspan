...
func NewAuthService(ctx context.Context, queries *db.Queries, pool *pgxpool.Pool, mailer email.Mailer, jwtSecret, hmacSecret, deploymentMode string, jwtTTL time.Duration) *AuthService {
...
func (s *AuthService) SetupStatus(ctx context.Context) (*SetupStatusResult, error) {
	if s.deploymentMode != "self_host" {
		return &SetupStatusResult{SetupComplete: true}, nil
	}
	count, err := s.queries.CountUsers(ctx)
	if err != nil {
		return nil, fmt.Errorf("setup status: count users: %w", err)
	}
	return &SetupStatusResult{SetupComplete: count > 0}, nil
}
...
func (s *AuthService) Register(ctx context.Context, emailAddr, name, password string) (*RegisterResult, error) {
	...
	link, err := s.mailer.SendVerification(emailAddr, name, rawToken, "en")
	if err != nil {
...
func (s *AuthService) RequestPasswordReset(ctx context.Context, emailAddr string) (*ResetRequestResult, error) {
	...
	// Send reset email
	link, err := s.mailer.SendPasswordReset(emailAddr, user.Name, rawToken, "en")
	if err != nil {
