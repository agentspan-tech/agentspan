...
import (
	...
	migrations "github.com/agentspan/processing/migrations"
...
func TestMain(m *testing.M) {
	...
	pgContainer, err := postgres.Run(ctx, "postgres:17-alpine",
		postgres.WithDatabase("agentspan_handler_test"),
...
func setupTestEnv(t *testing.T) *testEnv {
	...
	authSvc := service.NewAuthService(ctx, queries, pool, mailer, testJWTSecret, testHMACSecret, "cloud", 24*time.Hour)
	...
	authH := handler.NewAuthHandler(authSvc, mailer, "http://localhost:3000", 24*time.Hour, emailRL)
...
func registerAndLogin(t *testing.T, env *testEnv, email, name, password string) string {
	...
	result, err := env.authSvc.Register(ctx, email, name, password)
	if err != nil {
...
func TestHandler_Register(t *testing.T) {
	env := setupTestEnv(t)
	body := jsonBody(t, map[string]string{"email": "new@example.com", "name": "New User", "password": "Password1"})
...
func TestHandler_Register_InvalidEmail(t *testing.T) {
	env := setupTestEnv(t)
	body := jsonBody(t, map[string]string{"email": "not-an-email", "name": "User", "password": "Password1"})
	req := httptest.NewRequest("POST", "/auth/register", body)
	req.Header.Set("Content-Type", "application/json")
	rr := do(t, env, req)
...
func TestHandler_Register_WeakPassword(t *testing.T) {
	env := setupTestEnv(t)
	body := jsonBody(t, map[string]string{"email": "weak@example.com", "name": "User", "password": "short"})
	req := httptest.NewRequest("POST", "/auth/register", body)
	req.Header.Set("Content-Type", "application/json")
	rr := do(t, env, req)
	if rr.Code != http.StatusUnprocessableEntity {
...
func TestHandler_Login(t *testing.T) {
	...
	for _, c := range rr.Result().Cookies() {
		if c.Name == "agentspan_token" {
			found = true
		}
	}
	if !found {
		t.Error("expected agentspan_token cookie")
	}
}
...
func TestHandler_Auth_VerifyEmail(t *testing.T) {
	...
	body := jsonBody(t, map[string]string{"email": "vfy@example.com", "name": "Vfy User", "password": "Password1"})
	req := httptest.NewRequest("POST", "/auth/register", body)
	req.Header.Set("Content-Type", "application/json")
	rr := do(t, env, req)
	if rr.Code != http.StatusCreated {
	...
	// Extract token from URL: http://localhost:3000/auth/verify?token=XXX
...
func TestService_IntelligencePipeline_FailureCluster(t *testing.T) {
	...
	// Ingest 2 failed spans (HTTP 429) — need 2+ to get "failed" status (1 span = "abandoned")
	for i := 0; i < 2; i++ {
		err := internalSvc.IngestSpan(ctx, &service.SpanIngestRequest{
			APIKeyID:       apiKeyResult.ID.String(),
			OrganizationID: org.ID.String(),
...
func TestHandler_WebSocket_CookieAuth(t *testing.T) {
	...
	conn, _, err := websocket.Dial(ctx, wsURL, &websocket.DialOptions{
		HTTPHeader: http.Header{
			"Cookie": []string{"agentspan_token=" + token},
		},
	})
	if err != nil {
		t.Fatalf("ws dial: %v", err)
...
func TestHandler_WebSocket_CookieAuthInvalid(t *testing.T) {
	...
	conn, _, err := websocket.Dial(ctx, wsURL, &websocket.DialOptions{
		HTTPHeader: http.Header{
			"Cookie": []string{"agentspan_token=invalid-jwt-token"},
		},
	})
	if err != nil {
		t.Fatalf("ws dial: %v", err)
