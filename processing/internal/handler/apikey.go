...
func (h *APIKeyHandler) Routes() chi.Router {
	...
	// Test — owner, admin, member can test keys.
	r.With(h.requireRole("owner", "admin", "member")).Post("/{keyID}/test", h.Test)
...
func (h *APIKeyHandler) Deactivate(w http.ResponseWriter, r *http.Request) {
	...
	// Verify the key exists and belongs to this org before deactivating.
	if _, err := h.apiKeyService.GetAPIKey(r.Context(), orgID, keyID); err != nil {
		WriteError(w, http.StatusNotFound, "not_found", "API key not found")
		return
	}
...
// Test handles POST /api/orgs/{orgID}/keys/{keyID}/test.
// Sends a real request through the proxy to verify the key works end-to-end.
func (h *APIKeyHandler) Test(w http.ResponseWriter, r *http.Request) {
	orgID, ok := middleware.GetOrgID(r.Context())
	if !ok {
		WriteError(w, http.StatusInternalServerError, "internal_error", "Organization context missing")
		return
	...
	var req service.TestKeyRequest
	...
	result, err := h.apiKeyService.TestAPIKey(r.Context(), orgID, keyID, req.Model)
	if err != nil {
		...
		WriteError(w, http.StatusInternalServerError, "internal_error", "Failed to test API key")
		return
	}
