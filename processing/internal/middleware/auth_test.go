...
func TestAuthenticate_ValidJWT(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "agentspan_token", Value: token})
...
func TestAuthenticate_InvalidSignature(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/test", nil)
	req.AddCookie(&http.Cookie{Name: "agentspan_token", Value: signed})
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestAuthenticate_APIKey_Invalid(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/test", nil)
	req.Header.Set("Authorization", "Bearer as-"+crypto.HMACDigest("fake", testHMACSecret))
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
